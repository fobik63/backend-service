...
class CloudflareProtectionMiddleware(BaseHTTPMiddleware):
    ...
    VPN-gateway peers (Dead Man's Switch unlock path) bypass the CF edge check
    so admin recovery still works when both CF enforce and DMS lockdown are on.
    """
    ...
    async def dispatch(
        ...
        if settings.cloudflare_enabled and settings.cloudflare_enforce_edge:
            if path not in {"/health", "/health/live", "/healthz"}:
